...
class Premodel():
    def __init__(self, file):
        if file == 'Student':
            self.DATA_FILE = 'changed_local.csv' #new features only
        elif file == 'Author':
            self.DATA_FILE = 'changed_local.csv'
        else:
    ...
    def cv_training_data (self, amount_images):
        ...
        with open(self.DATA_FILE, 'rb') as csvfile:
            ...
            for row in csv.reader(lines):
                ...
                if row[2] != 1 and row[3] != 1:
                    data_lr.append(list(row[i] for i in [5, 10]))                   # changes what features will be used in the premodel. In this array features start at 4th index, and end at the 10th
                    data_dt.append(list(row[i] for i in [4, 9]))
        ...
        return  data_lr, data_dt, first_level, second_level, third_level
    ...
    def decision_tree(self, X_train, X_test, Y_train):
        ...
        regr_5 = DecisionTreeRegressor(max_depth=4, min_samples_leaf=10)
    ...
    def CV_fold_worker(self, test_idx, train_idx, img_data_lr, img_data_dt, first_level, second_level, third_level, first_level_machine, second_level_machine, third_level_machine, return_wrapper):
        """
        ...
        X_train, _ = util.list_split(img_data_lr, train_idx, [0])
        X_train_first_level = X_train
        X_train_second_level = X_train
        X_train_third_level = X_train


        Y_train, _ = util.list_split(img_data_lr, test_idx, [0])
...
#=============================================================================================================================
        X_train_dt, _ = util.list_split(img_data_dt, train_idx, [0])
        X_train_first_level_dt = X_train_dt
        X_train_second_level_dt = X_train_dt
        X_train_third_level_dt = X_train_dt


        Y_train_dt, _ = util.list_split(img_data_dt, test_idx, [0])
        Y_test_first_level_dt, _ = util.list_split(first_level, test_idx, [0])
        Y_test_second_level_dt, _ = util.list_split(second_level, test_idx, [0])
        Y_test_third_level_dt, _ = util.list_split(third_level, test_idx, [0])

        X_test_first_level_dt, _ = util.list_split(first_level, train_idx, [0])
        X_test_second_level_dt, _ = util.list_split(second_level, train_idx, [0])
        X_test_third_level_dt, _ = util.list_split(third_level, train_idx, [0])

        X_val_first_level_dt = [X_test_first_level_dt[i][1] for i in range(0,len(X_test_first_level_dt))]
        Y_val_first_level_dt = [Y_test_first_level_dt[i][1] for i in range(0,len(Y_test_first_level_dt))]

        X_val_second_level_dt = [X_test_second_level_dt[i][1] for i in range(0,len(X_test_second_level_dt))]
        Y_val_second_level_dt = [Y_test_second_level_dt[i][1] for i in range(0,len(Y_test_second_level_dt))]

        X_val_third_level_dt = [X_test_third_level_dt[i][1] for i in range(0,len(X_test_third_level_dt))]
        Y_val_third_level_dt = [Y_test_third_level_dt[i][1] for i in range(0,len(Y_test_third_level_dt))]

        for i in range(len(X_train_first_level_dt)):
            for j in range(len(X_train_first_level_dt[i])):
                X_train_first_level_dt[i][j] = float(X_train_first_level_dt[i][j])
        for i in range(len(X_val_first_level_dt)):
            X_val_first_level_dt[i] = int(X_val_first_level_dt[i])


        for i in range(len(X_train_second_level_dt)):
            for j in range(len(X_train_second_level_dt[i])):
                X_train_second_level_dt[i][j] = float(X_train_second_level_dt[i][j])
        for i in range(len(X_val_second_level_dt)):
            X_val_second_level_dt[i] = int(X_val_second_level_dt[i])



        for i in range(len(X_train_third_level_dt)):
            for j in range(len(X_train_third_level_dt[i])):
                X_train_third_level_dt[i][j] = float(X_train_third_level_dt[i][j])
        for i in range(len(X_val_third_level_dt)):
            X_val_third_level_dt[i] = int(X_val_third_level_dt[i])


        X_train_first_level_dt = np.array(X_train_first_level_dt)
        X_train_second_level_dt = np.array(X_train_second_level_dt)
        X_train_third_level_dt = np.array(X_train_third_level_dt)
        X_train_first_level_dt = X_train_first_level_dt.astype('float64')
        X_train_second_level_dt = X_train_second_level_dt.astype('float64')
        X_train_third_level_dt = X_train_third_level_dt.astype('float64')
        Y_train_dt = np.array(Y_train_dt)


        X_val_first_level_dt = np.array(X_val_first_level_dt)
        X_val_second_level_dt = np.array(X_val_second_level_dt)
        X_val_third_level_dt = np.array(X_val_third_level_dt)
        X_val_first_level_dt = X_val_first_level_dt.astype('int')
        X_val_second_level_dt = X_val_second_level_dt.astype('int')
        X_val_third_level_dt = X_val_third_level_dt.astype('int')
        ...
        undersample = NearMiss(version=1, n_neighbors=3)
    # transform the dataset
        X_train_first_level_dt, X_val_first_level_dt = undersample.fit_resample(X_train_first_level_dt, X_val_first_level_dt)

        X_train_second_level_dt, X_val_second_level_dt = undersample.fit_resample(X_train_second_level_dt, X_val_second_level_dt)

        X_train_third_level_dt, X_val_third_level_dt = undersample.fit_resample(X_train_third_level_dt, X_val_third_level_dt)
#=============================================================================================================================
        ...
        if first_level_machine == 'nn':
            ...
            predicted_level_2, predicted_level_5, predicted_level_8, predicted_level_12, predicted_level_16 = self.decision_tree(X_train_first_level_dt, X_val_first_level_dt, Y_train_dt)
            predicted = predicted_level_5
        ...
        for position, prediction in enumerate(predicted):
            if first_level_machine == 'dt16':
                if prediction > 0.5:
                    if Y_test_first_level_dt[position][1] == 1:
                        list_predictions.append((Y_test_first_level_dt[position][0], 1, prediction, 1, 'tf-mobilenet_v1'))
                    else:
                        list_predictions.append((Y_test_first_level_dt[position][0], 0, prediction, 1, 'tf-mobilenet_v1'))
                else:
        ...
        if second_level_machine == 'nn':
            ...
            predicted_level_2, predicted_level_5, predicted_level_8, predicted_level_12, predicted_level_16 = self.decision_tree(X_train_second_level_dt, X_val_second_level_dt, Y_train_second_level)
            predicted = predicted_level_5
            ...
            if second_level_machine == 'dt16':
                if prediction > 0.5:
                    if Y_test_second_level[position][1] == 1:
                        list_predictions.append((Y_test_first_level_dt[Y_train_second_level_position[position]][0], 2, prediction, 2, 'tf-inception_v4'))
                    else:
                        list_predictions.append((Y_test_first_level_dt[Y_train_second_level_position[position]][0], 0, prediction, 2, 'tf-inception_v4'))
                else:
        ...
        if third_level_machine == 'nn':
            ...
            predicted_level_2, predicted_level_5, predicted_level_8, predicted_level_12, predicted_level_16 = self.decision_tree(X_train_third_level_dt, X_val_third_level_dt, Y_train_third_level)
            predicted = predicted_level_5
            ...
            if third_level_machine == 'dt16':
                if prediction > 0.5:
                    if Y_test_third_level[position][1] == 1:
                        list_predictions.append((Y_test_first_level_dt[Y_train_third_level_position[position]][0], 3, prediction, 3, 'tf-resnet_v1_152'))
                    else:
                        list_predictions.append((Y_test_first_level_dt[Y_train_third_level_position[position]][0], 0, prediction, 3, 'tf-resnet_v1_152'))
                else:
                    if Y_test_third_level[position][1] == 1:
                        list_predictions.append((Y_test_first_level_dt[Y_train_third_level_position[position]][0], 3, prediction, 0, 'failed'))
                    else:
                        list_predictions.append((Y_test_first_level_dt[Y_train_third_level_position[position]][0], 0, prediction, 0, 'failed'))
            else:
                if prediction == 1:
                    if Y_test_third_level[position][1] == 1:
                        list_predictions.append((Y_test_first_level_dt[Y_train_third_level_position[position]][0], 3, prediction, 3, 'tf-resnet_v1_152'))
    ...
    def prototype(self, amount_images, list_premodels):
        ...
        data_dt, data_lr, first_level_data, second_level_data, third_level_data = self.cv_training_data(amount_images)
        ...
        for counter,(first_level_machine, second_level_machine, third_level_machine) in enumerate(list_premodels):
            ...
            k_fold = 10
            worker_threads = list()
            chunk_size = int(ceil(len(data_dt) / float(k_fold)))
            # Create a new thread for each fold
            for i, (test_idx, train_idx) in enumerate(util.chunkise(range(len(data_dt)), chunk_size)):
                return_wrapper = list()
                p = Thread(target=self.CV_fold_worker, args=(test_idx, train_idx, data_lr, data_dt, first_level_data, second_level_data, third_level_data, first_level_machine, second_level_machine, third_level_machine, return_wrapper))
                p.start()
        ...
        preds = list()
        for counter,(first_level_machine, second_level_machine, third_level_machine) in enumerate(list_premodels):
            preds.append(self.CV_fold_worker(test_idx, train_idx, data_lr, data_dt, first_level_data, second_level_data, third_level_data, first_level_machine, second_level_machine, third_level_machine, return_wrapper))
